...
settings_path = os.path.abspath(script_path + '/../../../settings/')
# Path to the player scripts directory
player_path = os.path.abspath(script_path + '/../../../scripts/')
...
def card_timeout_action() -> None:
    ...
    try:
        subprocess.run(f"{player_path}/playout_controls.sh -c=playerpauseforce -v=0.1", shell=True, check=False)
        pass
    except OSError as e:
        ...
        for card_id in reader:
            ...
            if card_id:
                if card_id != previous_id or (time.time() - previous_time) >= cfg_same_id_delay or card_id in cfg_cmd_cards:
                    ...
                    try:
                        subprocess.run(f"{player_path}/rfid_trigger_play.sh --cardid={card_id}", shell=True, check=False)
                        pass
                    except OSError as e:
