...
def gpio_controler():
    ...
    _gpio_controler = gpio_control(MockFunctionCalls)  # function_calls will be mocked
    return _gpio_controler
...
class TestGPIOControl:
    ...
    def test_getAllDevices_empty(self, gpio_controler):
        config = configparser.ConfigParser()
        devices = gpio_controler.get_all_devices(config)
        assert not devices

    def test_getAllDevices_empty_device(self, gpio_controler):
        name = 'TestDevice'
        config = configparser.ConfigParser()
        config[name] = {}
        devices = gpio_controler.get_all_devices(config)
    ...
    def test_getAllDevices_type_unknown(self, gpio_controler):
        name = 'TestDevice'
        config = configparser.ConfigParser()
        config[name] = {'enabled': 'true'}
        with patch.object(gpio_controler, gpio_controler.generate_device.__name__,
                          return_value=None) as mock_generate_device:
            devices = gpio_controler.get_all_devices(config)
            mock_generate_device.assert_called_once_with(config[name], name)
            assert not devices

    def test_getAllDevices_type_known(self, gpio_controler):
        name = 'TestDevice'
        config = configparser.ConfigParser()
        config[name] = {'enabled': 'true'}
        with patch.object(gpio_controler, gpio_controler.generate_device.__name__,
    ...
    def test_generateDevice_unknown(self, gpio_controler):
        name = 'TEST_unknown'
        config = configparser.ConfigParser()
        config[name] = {'Type': 'unknown', 'Pin': '5'}
        device = gpio_controler.generate_device(config[name], name)
        assert device is None

    def test_generateDevice_StatusLED(self, gpio_controler):
        name = 'TEST_StatusLED'
    ...
    def test_generateDevice_LED_default(self, gpio_controler):
        ...
        mock_init = func_test_generateDevice_type(gpio_controler, name, configArray, LED)
        mock_init.assert_called_once_with(5, name=name, initial_value=True)

    def test_generateDevice_LED(self, gpio_controler):
        name = 'TEST_LED'
        configArray = {'Type': 'LED', 'Pin': '5', 'initial_value': 'False'}
        mock_init = func_test_generateDevice_type(gpio_controler, name, configArray, LED)
        mock_init.assert_called_once_with(5, name=name, initial_value=False)

    def test_generateDevice_RotaryEncoder_default(self, gpio_controler):
        ...
        mock_init = func_test_generateDevice_type(gpio_controler, name, configArray, RotaryEncoder)
        mock_init.assert_called_once_with(5, 6, "test_funcCall1-None", "test_funcCall2-None", 0.1, name=name)

    def test_generateDevice_RotaryEncoder(self, gpio_controler):
        name = 'TEST_RotaryEncoder'
        configArray = {'Type': RotaryEncoder.__name__, 'Pin1': '5', 'Pin2': '6',
                        'functionCall1': 'test_funcCall1', 'functionCall2': 'test_funcCall2',
                        'timeBase': '1.1'}
    ...
    def test_generateDevice_SimpleButton_default(self, gpio_controler):
    ...
    def test_generateDevice_ShutdownButton_default(self, gpio_controler):
        name = 'TEST_ShutdownButton'
        configArray = {'Type': ShutdownButton.__name__, 'Pin': '5'}
        mock_init = func_test_generateDevice_type(gpio_controler, name, configArray, ShutdownButton)
    ...
    def test_generateDevice_ShutdownButton(self, gpio_controler):
        name = 'TEST_ShutdownButton'
        configArray = {'Type': ShutdownButton.__name__, 'Pin': '5',
                        'functionCall': 'test_funcCall1', 'functionCallArgs': 'test_funcCall1Args',
                        'bouncetime': 299, 'antibouncehack': 'True', 'edge': 'test_edge',
    ...
    def test_generateDevice_TwoButtonControl_default(self, gpio_controler):
        ...
        mock_init = func_test_generateDevice_type(gpio_controler, name, configArray, TwoButtonControl)
    ...
    def test_generateDevice_TwoButtonControl(self, gpio_controler):
        name = 'TEST_TwoButtonControl'
        configArray = {'Type': TwoButtonControl.__name__, 'Pin1': '5', 'Pin2': '6',
    ...
    def test_getFunctionCall_None_None(self, gpio_controler):
        result = gpio_controler.getFunctionCall(None, None)
        assert result(()) is None
        result = gpio_controler.getFunctionCall('None', None)
        assert result(()) is None
        result = gpio_controler.getFunctionCall("nonExisting", None)
        assert result(()) is None

    def test_getFunctionCall_withoutParam(self, gpio_controler):
        result = gpio_controler.getFunctionCall("funcTestWithoutParameter", None)
        assert result(()) == "funcTestWithoutParameter"

    def test_getFunctionCall_withParam(self, gpio_controler):
        result = gpio_controler.getFunctionCall("funcTestWithParameter", "param1")
        assert result(()) == "funcTestWithParameter(param1)"

    # ---------------

    def test_printAllDevices_empty(self, gpio_controler):
        with patch('builtins.print') as mock_print:
            gpio_controler.print_all_devices()
            mock_print.assert_not_called

    def test_printAllDevices_list(self, gpio_controler):
        with patch('builtins.print') as mock_print:
            gpio_controler.devices = ["test1", "test2"]
            gpio_controler.print_all_devices()
