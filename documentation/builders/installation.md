...
The Web App can upload, organize, and delete files in the audio library, so
Samba is disabled by default during installation. Choose Samba when you also
want direct network access to the complete `shared` directory, including
configuration files. See [Samba](samba.md) for details.
