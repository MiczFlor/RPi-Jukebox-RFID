...
install_jukebox_dependencies() {
  echo "Install Jukebox dependencies"
...
# Install Jukebox
install_jukebox() {
  echo "Install Jukebox"
  cd ${HOME_DIR}

  if [ -d "$INSTALLATION_DIR" ]; then
    cd ${INSTALLATION_DIR}
    if [[ `git status --porcelain` ]]; then
      echo "  Found local changes in git repository. Moving them to backup branch 'local-backup' and git stash"
      # Changes
      git fetch --all
