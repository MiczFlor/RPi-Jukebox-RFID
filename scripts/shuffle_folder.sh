...
# This script enables/disables shuffle mode for subfolder according to the folder.conf of the current folder/playlist
# Usage: 
# Enable shuffle for subfolder: ./shuffle_folder.sh -c=enableshuffle -v=foldername_in_audiofolders
# Disable resume for subfolder: ./shuffle_folder.sh -c=disableshuffle -v=foldername_in_audiofolders
#
# When to call this script?
# Call this script to switch play mode for subfolders
...
if [ "${DEBUG_shuffle_folder_sh}" == "TRUE" ]; then echo "## SCRIPT shuffle_folder.sh ($NOW) ##" >> $PATHDATA/../logs/debug.log; fi
if [ "${DEBUG_shuffle_folder_sh}" == "TRUE" ]; then echo "VAR AUDIOFOLDERSPATH: $AUDIOFOLDERSPATH" >> $PATHDATA/../logs/debug.log; fi
if [ "${DEBUG_shuffle_folder_sh}" == "TRUE" ]; then echo "VAR COMMAND: $COMMAND" >> $PATHDATA/../logs/debug.log; fi
if [ "${DEBUG_shuffle_folder_sh}" == "TRUE" ]; then echo "VAR VALUE: $VALUE" >> $PATHDATA/../logs/debug.log; fi
if [ "${DEBUG_shuffle_folder_sh}" == "TRUE" ]; then echo "VAR FOLDER: $FOLDER" >> $PATHDATA/../logs/debug.log; fi
...
then
    ...
    if [ "${DEBUG_shuffle_folder_sh}" == "TRUE" ]; then echo "  - calling inc.writeFolderConfig.sh -c=createDefaultFolderConf -d=\$FOLDER" >> $PATHDATA/../logs/debug.log; fi
    . $PATHDATA/inc.writeFolderConfig.sh -c=createDefaultFolderConf -d="$FOLDER"
    if [ "${DEBUG_shuffle_folder_sh}" == "TRUE" ]; then echo "  - back from inc.writeFolderConfig.sh" >> $PATHDATA/../logs/debug.log; fi
fi
# Read the current config file (include will execute == read)
. "$AUDIOFOLDERSPATH/$FOLDER/folder.conf"
if [ "${DEBUG_shuffle_folder_sh}" == "TRUE" ]; then echo "# found content in folder.conf:" >> $PATHDATA/../logs/debug.log; fi
if [ "${DEBUG_shuffle_folder_sh}" == "TRUE" ]; then cat "$AUDIOFOLDERSPATH/$FOLDER/folder.conf" >> $PATHDATA/../logs/debug.log; fi

if [ "${DEBUG_shuffle_folder_sh}" == "TRUE" ]; then echo "  Now doing what COMMAND wants: $COMMAND" >> $PATHDATA/../logs/debug.log; fi
...
# 	if [ $SHUFFLE == "ON" ];
# 	then 
# 		if [ "${DEBUG_shuffle_folder_sh}" == "TRUE" ]; then echo "  entering: shuffle_check with value $SHUFFLE" >> $PATHDATA/../logs/debug.log; fi
# 		mpc shuffle
# 	else
# 		if [ "${DEBUG_shuffle_folder_sh}" == "TRUE" ]; then echo "  entering: shuffle_check with value $SHUFFLE" >> $PATHDATA/../logs/debug.log; fi
# 		mpc random off
...
if [ "${DEBUG_shuffle_folder_sh}" == "TRUE" ]; then echo "# new content in folder.conf:" >> $PATHDATA/../logs/debug.log; fi
if [ "${DEBUG_shuffle_folder_sh}" == "TRUE" ]; then cat "$AUDIOFOLDERSPATH/$FOLDER/folder.conf" >> $PATHDATA/../logs/debug.log; fi
