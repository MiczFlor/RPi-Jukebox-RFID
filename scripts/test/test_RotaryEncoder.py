...
class TestRotaryEncoder:
    ...
    def test_start_stop(self, rotaryEncoder):
        ...
        MockRPi.GPIO.add_event_detect.assert_not_called()
        rotaryEncoder.start()
        assert rotaryEncoder.is_active is True
        assert MockRPi.GPIO.add_event_detect.call_count == 2
        MockRPi.GPIO.remove_event_detect.assert_not_called()
        rotaryEncoder.stop()
        assert MockRPi.GPIO.remove_event_detect.call_count == 2
        assert rotaryEncoder.is_active is False

    def test_Callback_Decr(self, rotaryEncoder):
        values = [{pinA: True, pinB: False},{pinA: False, pinB: False},{pinA: False, pinB: True},{pinA: True, pinB: True}]
        for i in range(5):

            def side_effect(arg):
                return values[i%len(values)][arg]
            MockRPi.GPIO.input.side_effect = side_effect
            rotaryEncoder._Callback(1)
        mockCallDecr.assert_once_called()
        mockCallIncr.assert_not_called()

    def test_Callback_Incr(self, rotaryEncoder):
        values = [{pinA: True, pinB: False},
                  {pinA: True, pinB: True},
                  {pinA: False, pinB: True},
